...
class TripReviewArgs {
  const TripReviewArgs(
      {required this.pickup, required this.drop, required this.rideType});
...
class _TripReviewScreenState extends ConsumerState<TripReviewScreen> {
  ...
  LatLng get _pickupLL =>
      LatLng(widget.args.pickup.lat, widget.args.pickup.lng);
  ...
  Future<void> _reviewBooking() async {
    ...
    if (fare == null) {
      showError(
          context,
          _fareError ??
              'Fare unavailable for this vehicle right now. Try again.');
      return;
    }
    final scheduledLater =
        _scheduledAt.difference(DateTime.now()).inMinutes.abs() > 5;
  ...
  Widget build(BuildContext context) {
    ...
    // A plain fixed layout, not a Stack + DraggableScrollableSheet — that
    // combination let dragging the sheet visually cover/uncover the map
    // (reads as "the map scrolls") and, on web, dragging a sheet directly
    // over a live GoogleMap platform view is what produced the torn/tiled
    // map rendering: the map now sits in its own stable Expanded region that
    // never resizes, and "Review Booking" is a fixed footer below the
    // scrollable content instead of the last item inside it.
    return Scaffold(
      body: LoadingOverlay(
        busy: _busy,
                ...
                children: [
                  Positioned.fill(
                    child: MapView(
                      ...
                      initial: _pickupLL,
                      markers: {
                        if (MapView.isRealLatLng(
                            _pickupLL.latitude, _pickupLL.longitude))
                          Marker(
                        ...
                        if (MapView.isRealLatLng(
                            _dropLL.latitude, _dropLL.longitude))
                          Marker(
              ...
              child: Material(
                ...
                child: Column(
                  children: [
                    ...
                    // "Review Booking" stays fixed here, below the scrollable
                    // content above — never part of what scrolls, so it's
                    // always visible without having to scroll down to it.
                    SafeArea(
                      top: false,
                      child: Padding(
                        padding: const EdgeInsets.fromLTRB(16, 8, 16, 16),
                        child: PrimaryButton(
                            label: 'Review Booking', onPressed: _reviewBooking),
                      ),
                    ),
                  ],
                ),
...
class _ScheduleSheetState extends State<_ScheduleSheet> {
  ...
  Future<void> _pickDate() async {
    ...
    if (d != null) {
      setState(() => _value =
          DateTime(d.year, d.month, d.day, _value.hour, _value.minute));
    }
  }
  ...
  Future<void> _pickTime() async {
    ...
    if (t != null) {
      setState(() => _value =
          DateTime(_value.year, _value.month, _value.day, t.hour, t.minute));
    }
  }
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            const Text('When do you need the ride?',
